"""
Burgers' Equation: Method Comparison via Theorem-Based, Spectral, and PINN Approaches

This script solves the nonlinear Burgers' PDE:
    u_t + u·u_x = ν·u_xx
on the domain [0, π] using:
1. Theorem-Based Trial Function
2. Spectral Method
3. Physics-Informed Neural Network (PINN)

The results are compared at t = π, and MSEs are computed w.r.t. a Finite Difference reference solution.
"""
...
# ==== Spectral Solver ====
def spectral_burgers(u0, dt, steps, nu):
    u = u0.copy()
    for _ in range(steps):
        u_hat = dst(u, type=2, norm='ortho')
        k = np.arange(1, len(u_hat)+1)
        decay = np.exp(-nu * (k * np.pi)**2 * dt)
        u_hat *= decay
...
u_spectral = spectral_burgers(u0, dt, Nt, nu)
...
plt.xlabel("x")
plt.ylabel("u(x, t=π)")
plt.title("Burgers' Equation: Method Comparison at t=π")
plt.legend()
plt.tight_layout()
plt.savefig("burgers_comparison.png", dpi=300)
plt.show()
